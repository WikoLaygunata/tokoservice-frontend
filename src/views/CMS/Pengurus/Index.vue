<template>
	<div>
		<n-card title="Data Pengurus">
			<n-divider class="divider" />

			<n-space :size="12" vertical>
				<n-space justify="space-between">
					<can permission="create-officer">
		...
		render(row) {
			return h(
				NDropdown,
				{
					...
					options: [
						canDo("update-officer") && {
							label: "Edit",
							key: "action-edit",
							icon: () => h(Icon, { name: "tabler:pencil" })
						},
						canDo("delete-officer") && {
							label: "Delete",
							key: "action-delete",
...
const filterValues = ref({ is_active: null })
	...
	try {
		const { data } = await axios.get("officers", {
			params: {
				...
				filters: { is_active: filterValues.value.is_active }
			}
		})
...
const deleteData = async id => {
	try {
		const { data } = await axios.delete(`officers/${id}`)

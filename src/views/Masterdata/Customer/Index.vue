		...
		<n-card title="Data Pelanggan">
			...
			<n-space :size="12" vertical>
				<n-space justify="space-between">
					...
					<n-space>
						<n-input v-model:value="searchQuery" clearable placeholder="Cari nama, nomor HP, alamat...">
							<template #prefix>
		...
		render(row) {
			return h(
				NDropdown,
				{
					...
					options: [
						canDo("update-customer") && {
							label: "Edit",
							key: "action-edit",
							icon: () => h(Icon, { name: "tabler:pencil" })
						},
						canDo("delete-customer") && {
							label: "Hapus",
							key: "action-delete",
...
const fetchData = async () => {
	...
	try {
		...
		message.error(error.response?.data?.message || error.message || "Gagal memuat data pelanggan")
	} finally {
		loading.value = false
	}
}
...
const handleDataAction = (row: any, key: string) => {
	if (key === "action-edit") {
		...
		const d = dialog.warning({
			...
			content: `Apakah Anda yakin ingin menghapus pelanggan "${row.name}"?`,
			positiveText: "Hapus",
			negativeText: "Batal",
			...
			onPositiveClick: async () => {
				d.loading = true
				try {
					const { data } = await axios.delete(`customers/${row.id}`)
					message.success(data.message || "Pelanggan berhasil dihapus")
					fetchData()
				} catch (error: any) {
					message.error(error.response?.data?.message || error.message || "Gagal menghapus pelanggan")
				} finally {
					d.loading = false
				}
			}

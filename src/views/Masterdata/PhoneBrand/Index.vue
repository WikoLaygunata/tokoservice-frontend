<template>
	<div>
		<n-card title="Data Merek HP (Phone Brand)">
			<n-divider class="divider" />

			<n-space :size="12" vertical>
				<n-space justify="space-between">
					<can permission="create-phone_brand">
					...
					<n-space>
						<n-input v-model:value="searchQuery" clearable placeholder="Cari merek HP...">
							<template #prefix>
		...
		render(row) {
			return h(
				NDropdown,
				{
					...
					options: [
						canDo("update-phone_brand") && {
							label: "Edit",
							key: "action-edit",
							icon: () => h(Icon, { name: "tabler:pencil" })
						},
						canDo("delete-phone_brand") && {
							label: "Hapus",
							key: "action-delete",
...
const fetchData = async () => {
	loading.value = true
	try {
		const { data } = await axios.get("phone-brands", {
			params: {
		...
		message.error(error.response?.data?.message || error.message || "Gagal memuat data merek HP")
	} finally {
		loading.value = false
	}
}
...
const handleDataAction = (row: any, key: string) => {
	if (key === "action-edit") {
		...
		const d = dialog.warning({
			title: "Hapus Merek HP",
			content: `Apakah Anda yakin ingin menghapus merek "${row.name}"?`,
			positiveText: "Hapus",
			negativeText: "Batal",
			...
			onPositiveClick: async () => {
				d.loading = true
				try {
					const { data } = await axios.delete(`phone-brands/${row.id}`)
					message.success(data.message || "Merek HP berhasil dihapus")
					fetchData()
				} catch (error: any) {
					message.error(error.response?.data?.message || error.message || "Gagal menghapus merek HP")
				} finally {
					d.loading = false
				}
			}

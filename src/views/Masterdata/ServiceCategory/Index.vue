<template>
	<div>
		<n-card title="Data Kategori Servis & Sparepart">
			<n-divider class="divider" />

			<n-space :size="12" vertical>
				<n-space justify="space-between">
					<can permission="create-service_category">
					...
					<n-space>
						<n-input v-model:value="searchQuery" clearable placeholder="Cari kategori servis...">
							<template #prefix>
		...
		render(row) {
			return h(
				NDropdown,
				{
					...
					options: [
						canDo("update-service_category") && {
							label: "Edit",
							key: "action-edit",
							icon: () => h(Icon, { name: "tabler:pencil" })
						},
						canDo("delete-service_category") && {
							label: "Hapus",
							key: "action-delete",
...
const handleDataAction = (row: any, key: string) => {
	if (key === "action-edit") {
		...
		const d = dialog.warning({
			...
			content: `Apakah Anda yakin ingin menghapus kategori "${row.name}"?`,
			positiveText: "Hapus",
			negativeText: "Batal",
			...
			onPositiveClick: async () => {
				d.loading = true
				try {
					const { data } = await axios.delete(`service-categories/${row.id}`)
					message.success(data.message || "Kategori servis berhasil dihapus")
					fetchData()
				} catch (error: any) {
					message.error(error.response?.data?.message || error.message || "Gagal menghapus kategori servis")
				} finally {
					d.loading = false
				}
			}
